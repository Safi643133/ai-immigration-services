...
import { autoFillForm } from '@/lib/services/formAutoFill'
...
export default function FormsPage() {
  ...
  const { validationErrors, validateAndProceed, clearErrors } = useFormValidation()
  ...
  const handleTemplateSelect = async (template: FormTemplate) => {
    setSelectedTemplate(template)
    const filledData = await autoFillForm(template, extractedData)
                  ...
                  {selectedTemplate.form_type === 'ds160' ? (
                    <div>
                      <Stepper
                        ...
                        onNext={async () => {
                          await validateAndProceed(ds160Step, formData, (nextStep) => {
                            setDs160Step(Math.min(17, nextStep))
                          })
                        }}
                      />
                                              {ds160Step === 1 && (
                          <FormStep 
                            title="Personal Information - Part 1"
                      ...
                      {ds160Step === 2 && (
                        <FormStep 
                          title="Personal Information - Part 2"
                          validationErrors={validationErrors}
                        >
